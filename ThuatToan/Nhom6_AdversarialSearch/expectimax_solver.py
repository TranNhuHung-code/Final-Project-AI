...
class ExpectimaxSudokuBattle:
    """
    SUDOKU BATTLE (Luật Mới: ÉP ĐỐI THỦ GIẢI Ô CHỈ ĐỊNH) - Tối ưu với Expectimax
    """
    ...
    def _expectimax(self, board, depth, is_max_turn):
        ...
        if is_max_turn:
            ...
            for r, c in empty_cells:
                ...
                val = hardness + self._expectimax(board, depth - 1, False)
                board[r][c] = 0
                
                if val > best_val: best_val = val
            return best_val
        else:
            # Chance node: assume Human picks uniformly at random from remaining cells
            total_val = 0
            for r, c in empty_cells:
                hardness = self._hardness(board, r, c)
                board[r][c] = self.real_solution[r][c]
                val = -hardness + self._expectimax(board, depth - 1, True)
                board[r][c] = 0
                total_val += val
            return total_val / len(empty_cells)
    ...
    def agent_choose_target(self):
        ...
        for r, c, hardness in candidates:
            ...
            val = hardness + self._expectimax(self.board, self.search_depth - 1, False)
            self.board[r][c] = 0
