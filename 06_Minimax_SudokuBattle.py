...
COLOR_SELECTED_BG = "#cfe8ff"
COLOR_HUMAN_CORRECT_BG = "#d6f5d6"
COLOR_HUMAN_CORRECT_TEXT = "#1b5e20"
COLOR_HUMAN_WRONG_BG = "#ffd6d6"
COLOR_HUMAN_WRONG_TEXT = "#b3261e"
COLOR_AGENT_CORRECT_BG = "#bfe3ff"
...
class MinimaxBattleApp:
    ...
    # ------------------------------------------------------------------
    def _build_ui(self):
        title = tk.Label(self.root, text="🎮 SUDOKU BATTLE — Minimax | Người vs Agent",
                          font=FONT_TITLE, bg=COLOR_BG, fg="white")
        title.pack(pady=(12, 4))

        subtitle = tk.Label(
            self.root,
            text="Nhóm: Adversarial Search   |   Đồ án cuối kỳ môn Trí Tuệ Nhân Tạo",
        ...
        grid_frame = tk.Frame(left_frame, bg="#000000", bd=2)
        grid_frame.pack()
        ...
        # ----- Khung điểm số -----
        score_frame = tk.Frame(left_frame, bg=COLOR_BG)
        score_frame.pack(pady=10, fill="x")

        self.human_score_label = tk.Label(score_frame, text="👤 Người: 0",
                                           font=FONT_SCORE, bg=COLOR_BG, fg="#34d399")
        self.human_score_label.pack(side="left", padx=10)

        self.agent_score_label = tk.Label(score_frame, text="🤖 Agent: 0",
                                           font=FONT_SCORE, bg=COLOR_BG, fg="#60a5fa")
        self.agent_score_label.pack(side="left", padx=10)

        # ----- Khung nhập liệu -----
        input_frame = tk.Frame(left_frame, bg=COLOR_BG)
        input_frame.pack(pady=8)

        tk.Label(input_frame, text="Ô đã chọn:", font=FONT_LABEL,
                 bg=COLOR_BG, fg="#cbd2d9").grid(row=0, column=0, padx=4)
        self.selected_cell_label = tk.Label(input_frame, text="(chưa chọn)",
                                             font=FONT_LABEL, bg=COLOR_BG, fg="white")
        self.selected_cell_label.grid(row=0, column=1, padx=4)

        tk.Label(input_frame, text="Số (1-9):", font=FONT_LABEL,
                 bg=COLOR_BG, fg="#cbd2d9").grid(row=0, column=2, padx=4)
        self.value_entry = tk.Entry(input_frame, width=3, font=FONT_LABEL)
        ...
        self.btn_confirm = tk.Button(input_frame, text="✓ Xác nhận điền số",
                                      ...
                                      activebackground="#059669", relief="flat", padx=10, pady=4)
        ...
        self.info_label = tk.Label(left_frame, text="Lượt của Người! Click chọn 1 ô trống, nhập số rồi xác nhận.",
        ...
        tk.Label(right_frame, text="🧠 Log suy luận Minimax của Agent",
                 font=("Segoe UI", 12, "bold"), bg="#111827", fg="white").pack(pady=(8, 4), padx=10)

        self.log_text = tk.Text(right_frame, width=42, height=28, bg="#0b0f14", fg="#9cdcfe",
