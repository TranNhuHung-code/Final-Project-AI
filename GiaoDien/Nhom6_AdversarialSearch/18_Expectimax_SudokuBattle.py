...
LUẬT CHƠI "SUDOKU BATTLE" (PEAS):
    ...
    - Đây là trò chơi 2 NGƯỜI, LUÂN PHIÊN LƯỢT: Agent đóng vai MAX (tối 
      đa hóa hiệu số điểm), Người được giả định đóng vai CHANCE (đi ngẫu 
      nhiên) trong quá trình Agent TÍNH TOÁN nước đi.
    - Vì cây trò chơi quá lớn để duyệt hết, Agent dùng EXPECTIMAX CÓ GIỚI
      HẠN ĐỘ SÂU + HÀM ĐÁNH GIÁ ở node lá. 
      Chi tiết: xem file expectimax_solver.py (ExpectimaxSudokuBattle).
...
CÁCH CHƠI:
    ...
    3. Đến lượt Agent: Agent tự động dùng Expectimax để chọn (ô, số) tốt
...
CÁCH CHẠY:
    python 18_Expectimax_SudokuBattle.py
...
from ThuatToan.Nhom6_AdversarialSearch.expectimax_solver import ExpectimaxSudokuBattle
...
class ExpectimaxBattleApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Sudoku Battle - Expectimax (Người vs Agent)")
        ...
        self.game = ExpectimaxSudokuBattle(self.puzzle, self.real_solution,
                                         search_depth=2, candidate_cells_per_turn=5)
    ...
    # ------------------------------------------------------------------
    def _build_ui(self):
        title = tk.Label(self.root, text="Sudoku Battle: You vs Expectimax AI",
        ...
        self.human_score_label = tk.Label(score_frame, text="👤 Người: 0",
                                   font=FONT_TITLE, bg=CARD, fg=COLOR_P1_BG)
        self.human_score_label.pack(side="left", padx=20, pady=10)

        self.agent_score_label = tk.Label(score_frame, text="🤖 Agent: 0",
        ...
        tk.Label(right_frame, text="🧠 Log suy luận Expectimax của Agent",
    ...
    def _mark_cell_result(self, row, col, is_correct, by_agent):
        ...
        if by_agent:
            ...
            fg = COLOR_HUMAN_CORRECT_TEXT if is_correct else COLOR_HUMAN_WRONG_TEXT
    ...
    def on_confirm_human_move(self):
        ...
        self.info_label.config(text="Lượt của Agent... Agent đang suy luận bằng Expectimax.")
    ...
    def _on_agent_move_done(self, row, col, value, is_correct, trace, elapsed):
        ...
        self._log(f"🤖 Agent (Expectimax, độ sâu={self.game.search_depth}) suy luận trong {elapsed:.3f}s, "
