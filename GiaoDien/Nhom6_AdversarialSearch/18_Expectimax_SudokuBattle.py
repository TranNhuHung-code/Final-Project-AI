...
class ExpectimaxBattleApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Sudoku Battle - Expectimax (Luật Ép Ô)")
    ...
    def _build_ui(self):
        title = tk.Label(self.root, text="Sudoku Battle: You vs Expectimax AI (Luật Ép Ô)",
    ...
    def _start_game_with_puzzle(self):
        self.game = ExpectimaxSudokuBattle(self.puzzle, self.real_solution, search_depth=3)
        self.moves_history = []
    ...
    def _on_agent_move_done(self, solved_r, solved_c, solved_val, next_r, next_c, trace, nodes, elapsed):
        ...
        self._log(f"🤖 Agent (Expectimax, depth={self.game.search_depth}) suy luận trong {elapsed:.3f}s:")
        self._log(f"   ➜ Agent ĐÃ GIẢI ĐÚNG ô ({solved_r+1}, {solved_c+1}) = {solved_val}")

        if next_r is not None:
            self._log_tree("==================================================")
            self._log_tree(f"🤖 ĐẾN LƯỢT AI CHỌN Ô CHO BẠN")
            self._log_tree(f"- Thuật toán: Expectimax")
