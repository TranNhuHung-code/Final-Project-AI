...
COLOR_TARGET_BG = "#FFD700"  # Vàng chóe cho ô bị ép
...
class MinimaxBattleApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Sudoku Battle - Minimax (Luật Ép Ô)")
    ...
    # ------------------------------------------------------------------
    def _build_ui(self):
        title = tk.Label(self.root, text="Sudoku Battle: You vs Minimax AI (Luật Ép Ô)",
        ...
        # Ẩn nút lùi nước vì luật chơi chéo cờ phức tạp
    ...
    def _start_game_with_puzzle(self):
        self.game = MinimaxSudokuBattle(self.puzzle, self.real_solution, search_depth=2)
        self.moves_history = []
        self.selected_cell = None
        self.target_cell = None
        self.phase = "HUMAN_CHOOSE_TARGET" # "HUMAN_CHOOSE_TARGET" or "HUMAN_FILL"
    ...
    def on_cell_click(self, row, col):
        ...
        if self.phase == "HUMAN_CHOOSE_TARGET":
            self.selected_cell = (row, col)
            self._render_board()
            # Tự động chốt ô này cho AI luôn để nhanh gọn
    ...
    def _on_agent_move_done(self, solved_r, solved_c, solved_val, next_r, next_c, trace, nodes, elapsed):
        ...
        self._log(f"🤖 Agent (Minimax, depth={self.game.search_depth}) suy luận trong {elapsed:.3f}s:")
        self._log(f"   ➜ Agent ĐÃ GIẢI ĐÚNG ô ({solved_r+1}, {solved_c+1}) = {solved_val}")

        if next_r is not None:
            self._log_tree("==================================================")
            self._log_tree(f"🤖 ĐẾN LƯỢT AI CHỌN Ô CHO BẠN")
            self._log_tree(f"- Thuật toán: Minimax")
            ...
            for t in trace:
                self._log_tree(f"  + Ô ({t['row']+1}, {t['col']+1}): Độ khó = {t.get('hardness', 0)} | Điểm = {t['score']}")
